...
impl TreeNodeRewriter for Simplifier<'_> {
    ...
    fn f_up(&mut self, expr: Expr) -> Result<Transformed<Expr>> {
        ...
        Ok(match expr {
            ...
            Expr::BinaryExpr(BinaryExpr { left, op, right })
                if supports_cast_predicate_for_binary(info, &right, op, &left)
                    && op.supports_propagation()
                    && op.swap().is_some() =>
            {
            ...
            Expr::InList(InList {
                ...
                list,
                negated,
            }) if supports_cast_predicate_for_inlist(info, &left, &list) => {
                rewrite_cast_predicate_for_inlist(info, *left, list, negated)?
            }
